/* ============================================================
   Mira Reflection Companion — Cloudflare Worker proxy
   ------------------------------------------------------------
   Keeps your AI API key SERVER-SIDE. Students never see it.
   Access is gated by a shared class passcode.

   Set these as Worker secrets/variables (see DEPLOY guide):
     CLASS_PASSCODE     (required)  e.g. "reflect2026"
     OPENAI_API_KEY     (set if you use OpenAI)
     ANTHROPIC_API_KEY  (set if you use Claude)
     OPENAI_MODEL       (optional)  default gpt-4o-mini
     ANTHROPIC_MODEL    (optional)  default claude-3-5-haiku-latest
     ALLOWED_ORIGIN     (optional)  e.g. https://yoursite.com  (default "*")
     DEFAULT_PROVIDER   (optional)  "openai" or "anthropic"
   ============================================================ */
...
4. Offer resources:
   - [INSERT YOUR INSTITUTION'S RESOURCES HERE — e.g., university counseling center, student affairs, an advisor's contact. Replace this line before sharing widely.]
   - Thailand: Department of Mental Health hotline **1323** (24 hours). (Please verify and localize for your students.)
...
export default {
  async fetch(request, env) {
    ...
    // --- Passcode gate ---
    if (!env.CLASS_PASSCODE) return json({ error: "Server not configured: missing CLASS_PASSCODE." }, 500, cors);
    if (!body.passcode || body.passcode !== env.CLASS_PASSCODE)
      return json({ error: "Invalid or missing class passcode." }, 401, cors);

    // --- Inputs ---
    let provider = body.provider === "anthropic" ? "anthropic" : body.provider === "openai" ? "openai" : (env.DEFAULT_PROVIDER || "openai");
    ...
    if (totalChars > 24000) return json({ error: "Conversation too long. Please start a new session." }, 413, cors);
    messages = messages
    ...
    try {
      if (provider === "anthropic") {
        ...
        if (!key) return json({ error: "Server missing ANTHROPIC_API_KEY." }, 500, cors);
        ...
        if (!r.ok) return json({ error: "AI service error (" + r.status + ")." }, 502, cors);
        const data = await r.json();
        const reply = (data.content || []).map((c) => c.text || "").join("").trim();
        return json({ reply }, 200, cors);
      } else {
        const key = env.OPENAI_API_KEY;
        if (!key) return json({ error: "Server missing OPENAI_API_KEY." }, 500, cors);
